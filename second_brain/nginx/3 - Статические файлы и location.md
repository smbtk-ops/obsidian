...
Встроенная переменная Nginx — нормализованный URI текущего запроса **без query-параметров**. Значение может меняться в процессе обработки (после `rewrite`, внутренних редиректов).

Нормализация — nginx приводит URI к чистому виду перед записью в `$uri`:

| Что делает | Было (в запросе) | Стало в `$uri` |
|-----------|------------------|----------------|
| Декодирует спецсимволы (`%20` = пробел) | `/my%20file.txt` | `/my file.txt` |
| Раскрывает `..` (подняться на уровень выше) | `/a/b/../c.css` | `/a/c.css` |
| Убирает лишние `/` | `//shop///page` | `/shop/page` |
| Отбрасывает query-параметры | `/page?sort=price` | `/page` |

> `..` работает как в терминале: `/a/b/../c.css` — зашли в `b`, вышли обратно в `a` (`..`), взяли `c.css`. Результат — `/a/c.css`.

Не путать с `$request_uri` — оригинальный URI **с query-параметрами**, без нормализации, не изменяется после rewrite:

```
Запрос: GET /shop/games/../page?sort=price HTTP/1.1

$request_uri = /shop/games/../page?sort=price
$uri          = /shop/page
```

| | `$uri` | `$request_uri` |
|---|--------|----------------|
| Query-параметры | Нет | Да |
| Нормализация | Да | Нет |
| Изменяется после rewrite | Да | Нет |
| Использование | `try_files`, `rewrite`, внутренняя логика | Логирование, передача оригинального запроса |

---

## 5. Директива try_files
...
### 5.1. Базовый пример — SPA приложение
...
### 5.3. Проксирование в несколько S3 бакетов
...
### 5.4. Возврат кода ошибки как fallback
...
## 6. Порядок обработки статического запроса
