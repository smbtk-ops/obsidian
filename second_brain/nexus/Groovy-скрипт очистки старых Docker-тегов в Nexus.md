...
## 2. Основные изменения и улучшения

| Раздел                      | Что сделано                                              | Причина                                  |
| --------------------------- | -------------------------------------------------------- | ---------------------------------------- |
| Чтение конфигурации         | Креды читаются из внешнего файла `scripts.env`            | Чтобы не хранить пароли в коде           |
| Аутентификация              | Авторизация через HTTP Basic                              | Универсальный метод для REST API Nexus   |
| Постраничная загрузка       | Добавлен цикл с `continuationToken`                       | Nexus возвращает ассеты постранично      |
| Группировка ассетов         | Ассеты группируются по имени образа                       | Чтобы вычислить старые теги для каждого образа |
...
## 5. Настройки, которые можно менять
...
2. Убедись, что строки вида `Образ 'slot-front': всего тегов=12, сохраняем=5, удаляем=7` присутствуют.
